...
fn test_consistent(){
    ...
    let res1: Vec<String> = find_consistent(&vec![r1], &ES).into_iter().map(|gid|ES.resolve_gene_id(gid)).collect();
    ...
    let res2: Vec<String> = find_consistent(&vec![r2], &ES).into_iter().map(|gid|ES.resolve_gene_id(gid)).collect();
    ...
    let res3: Vec<String> = find_consistent(&vec![r3, r4], &ES).into_iter().map(|gid|ES.resolve_gene_id(gid)).collect();
    ...
    let res4: Vec<String> = find_consistent(&vec![r5, r6], &ES).into_iter().map(|gid|ES.resolve_gene_id(gid)).collect();
    ...
    let res5: Vec<String> = find_consistent(&vec![r7, r8, r9], &ES).into_iter().map(|gid|ES.resolve_gene_id(gid)).collect();
...
fn ec_combinatorics_for_gene(geneid: u32, ec2geneid: &HashMap<EC, HashSet<u32>>){
    ...
    while  current_maybe_sets.len() > 0{
        ...
        for (ec, set) in current_maybe_sets{
            let overlap = set.intersection(&target_set).collect::<Vec<&u32>>().len();
            if overlap > 0{
